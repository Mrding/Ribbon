	...
	public class AttachedEventTriggerParser : ITriggerParser
	{
		...
		protected virtual RoutedEvent GetRoutedEvent(DependencyObject target, string triggerText)
		{
			...
			if (triggerText.Contains("."))
			{
			    ...
			    var assemblies = GetSearchableAssemblies().ToList();
                var nhddl = new[] { "HibernatingRhinos.Profiler.Appender", "Mono.Cecil.Pdb", "Mono.Cecil.Mdb","System.Web.Services", "System.ServiceModel", "Caliburn.ModelFramework", "WPFToolkit" };
                foreach (var nh in nhddl)
                {
                    var hasnh = assemblies.FirstOrDefault(o => o.FullName.Contains(nh));
                    if (hasnh == null)
                        break;
                    assemblies.Remove(hasnh);
                }
#if NET35
                var types = assemblies.SelectMany(a => a.GetExportedTypes())
                    .Where(x => _dependencyObjectType.IsAssignableFrom(x) || (x.IsAbstract && x.IsSealed || typeof(Binding).IsAssignableFrom(x)));
#endif
#if NET40
                var types = assemblies.Where(a => a.GetType().FullName == "System.Reflection.RuntimeAssembly").SelectMany(a => a.GetExportedTypes())

...
namespace Caliburn.Castle
{
    ...
    public static class WindsorContainerExtensions
    {
        ...
        public static T TryResolve<T>(this IWindsorContainer container, string key)
        {
            var type = typeof(T);

            if (container.Kernel.HasComponent(type) && container.Kernel.HasComponent(key))
        ...
        public static T TryResolve<T>(this IWindsorContainer container, IDictionary<string, object> args)
        {
            if (typeof(T).IsClass && !container.Kernel.HasComponent(typeof(T)))
                container.Kernel.AddComponent(typeof(T).FullName, typeof(T), LifestyleType.Transient);

            object result = container.Resolve(typeof(T), args);
            if (result != null)
            {
                return (T)result;
            }
